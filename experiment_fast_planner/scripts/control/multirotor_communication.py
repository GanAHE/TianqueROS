...
class Communication:
    ...
    def __init__(self):
        ...
        self.cmd_sub = rospy.Subscriber("planning/ref_traj",String,self.cmd_callback)
        self.cmd_pose_flu_sub = rospy.Subscriber("/cmd_pose_flu", Pose, self.cmd_pose_flu_callback)
        self.cmd_pose_enu_sub = rospy.Subscriber("/cmd_pose_enu", Pose, self.cmd_pose_enu_callback)
        self.cmd_vel_flu_sub = rospy.Subscriber("/cmd_vel_flu", Twist, self.cmd_vel_flu_callback)
        self.cmd_vel_enu_sub = rospy.Subscriber("/cmd_vel_enu", Twist, self.cmd_vel_enu_callback)
        self.cmd_accel_flu_sub = rospy.Subscriber("/cmd_accel_flu", Twist, self.cmd_accel_flu_callback)
        self.cmd_accel_enu_sub = rospy.Subscriber("/cmd_accel_enu", Twist, self.cmd_accel_enu_callback)
            
        ''' 
        ros publishers
        '''
        self.target_motion_pub = rospy.Publisher("/mavros/setpoint_raw/local", PositionTarget, queue_size=10)
        self.odom_groundtruth_pub = rospy.Publisher('/ground_truth/odom', Odometry, queue_size=10)
        ...
        self.gazeboModelstate = rospy.ServiceProxy('mavros/state', GetModelState)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                response = self.gazeboModelstate ('ground_plane')
